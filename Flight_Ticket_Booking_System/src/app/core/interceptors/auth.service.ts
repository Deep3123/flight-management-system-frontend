...
export class AuthInterceptor implements HttpInterceptor {
    ...
    return next.handle(clonedRequest).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.error && typeof error.error === "string") {
          ...
          try {
            const decrypted = this.encryptionService.decrypt(error.error);
            let parsedBody;
            try {
            ...
            // Create a new error response with decrypted body
            ...
            this.handleSpecificErrors(decryptedError);
        ...
        // Handle the case where Angular tried to parse the response as JSON but it's actually encrypted
        else if (error.error && error.error.text) {
          try {
            const decrypted = this.encryptionService.decrypt(error.error.text);
            let parsedBody;
            try {
              parsedBody = JSON.parse(decrypted);
            } catch (jsonError) {
            ...
            console.error("Failed to decrypt error.text response:", e);
          }
        }

        // Process specific error codes for non-encrypted responses
  ...
  // Separate method to handle specific HTTP error codes
